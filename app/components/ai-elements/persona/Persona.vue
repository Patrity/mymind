...
// MyMind patch (cycle 65 Task 0 spike): point Rive's runtime at our own origin
// (Nitro `publicAssets` `rive`, nuxt.config.ts) instead of its default jsdelivr CDN
// fetch, BEFORE the first `new Rive(...)` call below. Module-scope so it only runs
// once per bundle load; guarded for SSR since this file can be statically imported
// even though the component itself is only ever mounted inside <ClientOnly>.
...
const source = computed(() => {
  const base = sources[props.variant]
  if (props.srcOverride)
    return { ...base, dynamicColor: false, hasModel: false, source: props.srcOverride }
  return base
})
