...
**One toolbar (`app/components/agent/Toolbar.vue`).** Carries the **current thread title** (falling back to "Bridget"), a full-screen (voice mode) button, an `lg:hidden` threads button, and an `#actions` slot the page fills with `VoiceSettingsSlideover`. **Cycle 65 moved the voice-replies switch and the reasoning-model selector OUT of it and into the composer** — which also retires the cycle-60 `hidden sm:flex` workaround that kept them from pushing the toolbar off-screen at 375 px. Voice replies is still mirrored in `VoiceSettingsSlideover`, bound to the same `agent-speak` cookie ref (one source of truth, no drift).
...
- **Variants: `obsidian` (default), `mana`, `opal`, `glint`.** Picked in `VoiceSettingsSlideover` and cookie-persisted with the other voice settings (`useVoiceSettings().settings.personaVariant`; an unknown or retired value normalizes to `obsidian`). Upstream also ships `halo` and `command` — **both were removed from the picker in cycle 65 because they render pure white regardless of theme** (their `dynamicColor` view model doesn't respond), i.e. invisible in light mode.
- **Assets.** Rive's **wasm** is served from our own origin through a Nitro `publicAssets` entry (`baseURL: 'rive'`, pointed at the `@rive-app/webgl2` package dir); `Persona.vue` calls `RuntimeLoader.setWasmUrl('/rive/rive.wasm')` before the first `new Rive`. The four **`.riv` files** are committed under `public/persona-riv/` and the vendored sources map points at those local paths — cycle 65 chose to own them rather than depend at runtime on Vercel's blob host for the agent surface's core visual. **Naming trap:** the directory cannot be named `rive-*`. The wasm `publicAssets` entry's `rive` baseURL **prefix-matches**, so `public/rive-personas/*.riv` 404s.
...
- **An audio-reactive Persona** (cycle 65): the Persona is **state-driven only** (`idle`/`listening`/`thinking`/`speaking`/`asleep`). `micAnalyser`/`outAnalyser` still exist on `useVoice`, but only `AgentMicBand` reads them.
