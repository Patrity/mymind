...
shipped:
  ...
  - "app/pages/voice.vue — passes state/connected/micAnalyser/onVizEvent to Reactor; activeAnalyser helper removed. Acceptance additions: opaque caption over the canvas showing the current turn's message (bg-elevated, line-clamp-3); two sibling UDashboardPanels (documents pattern): canvas panel resizable/sized in rem (default 48, min 30, max 120 — the layout's UDashboardGroup uses unit=rem; double-click the handle to reset), transcript panel fluid on the right, hidden below lg; both bodies use :ui body !p-0 so the Three.js canvas fills the panel edge-to-edge. NOTE: Nuxt UI's resize handle only supports a sized panel LEFT of the handle (no side prop on UDashboardPanel), which is why the canvas is the sized one."
