...
- `server/api/capture/transcribe.post.ts` — `{ imageId, title? }` → `getImage` → blob → data URL → `describeImage` → `createDoc({ path:'/input/transcribed-<nanoid>.md', content: ocrText })`. Blank/no-text images create a stub gracefully.
