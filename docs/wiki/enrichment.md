...
- Proposer `server/lib/ai/enrich.ts`: `proposeFrontmatter(doc)` asks the reasoning model for STRICT JSON (`title/project/domain/type/tags/path/reasoning`); `parseProposal(raw)` tolerates code fences / surrounding prose / coerces tags, returns null on failure.
