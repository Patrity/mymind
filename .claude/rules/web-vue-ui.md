...
- **Validate UI work with `playwright-cli`, NOT the Playwright MCP.**
  - **Local dev test account** (auth-gated pages like `/settings`): the dev DB is single-tenant and usually fresh — register the first user via the login page's "Create account". Standing creds: **`test@example.com`** / **`testpassword123`** (name "Test User"). Reuse these; re-register if the dev DB was reset.
  - **reka-ui tabs/toggles** (`UTabs`, segmented controls): drive them with `playwright-cli click <e-ref>` (a real click), NOT a programmatic `el.click()` in `eval` — the latter doesn't fire reka's handler. See the `playwright-cli-reka-tabs` memory.
