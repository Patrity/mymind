...
- **Session ↔ project reassignment gap — ✅ closed (cycle 46, 2026-07-15, built on `feat/session-project-reassignment`, not yet merged).** Sessions can now be moved to any project (single or bulk UI), which cascades their agent-scoped memories and can teach the router a new `path_prefixes` root so the same folder auto-routes correctly next time. Remaining **operational** steps (not code): merge the branch; run `scripts/reresolve-uncategorized.ts` against prod (existing-projects-only re-resolve; dev dry-run moved 14/399); then, in the UI, create a `Terawulf` project, bulk-move its sessions (the largest uncategorized cluster, machine `Tony-NLS`), and register its path prefix — drains the backlog and validates auto-routing live. See [`wiki/sessions.md`](wiki/sessions.md) + [`wiki/projects.md`](wiki/projects.md) + the [cycle-46 handover](handovers/2026-07-15-session-project-reassignment.md).
