...
- **`hidden sm:flex` on the wide controls is necessary, not cosmetic.** Without it, at 375 px the
  ...
  **off-screen** — removing the only route to threads on a phone. Because hiding a *voice* agent's
  ...
  `AgentSettingsSlideover`** (named `VoiceSettingsSlideover` at the time; cycle 67 moved the file to
  `app/components/agent/`, which renamed its auto-import), bound to the same `agent-speak` cookie ref
  (bidirectional sync re-verified with no drift).
...
| R4 | **Task 6 keeps `<AgentSettingsSlideover :voice>` self-contained**; no `v-model:open`, no `@settings` | The component takes only `{ voice }` and renders its own cog trigger; an `open` model is scope the spec never asked for | The settings trigger sits in the toolbar rather than being page-controlled — cosmetic |
