...
| `dynamic-tool` | `app/components/agent/ToolPart.vue` → Elements `Tool`/`ToolHeader`/`ToolInput`/`ToolOutput` — a status badge (**Running** while `input-streaming`\|`input-available`, **Completed**/**Error**/**Denied** once settled), a collapsible JSON input (`CodeBlock`) and output (`envelope.value`, syntax-highlighted), and an **Undo** action when `envelope.undoToken` is present |
