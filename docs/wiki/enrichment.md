...
- `review_queue` table: `id, doc_id (deprecated, nullable, no longer written — cycle-70 task-7), target_kind ('document'|'memory'), target_id, kind, proposed jsonb, status (pending|approved|rejected), created_at, resolved_at`. `target_kind`/`target_id` are the honest polymorphic reference — `doc_id` had silently held a `memories.id` under `memory-supersede`/`memory-contradict` kinds since those existed; the DROP is deferred (prod is live).
