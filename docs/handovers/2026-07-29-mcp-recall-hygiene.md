...
cycle: 51
date: 2026-07-29
status: ✅ SHIPPED — merged to master (fast-forward, `e4f5a9e..dc06f1e`), pushed, and DEPLOYED via CD run 30573162535 (green, exit 0). Prod verified independently: LXC 114 `hostname` → `mymind`, `systemctl is-active mymind` → active, DB-touching `/api/health` → 200. Live prod MCP round-trip confirms the fix at production scale (see "Prod acceptance" below). Gates at merge: typecheck 0 / test 934 across 133 files / build clean. ✅ FULLY COMPLETE. `scripts/collapse-local-paths.ts --apply` HAS been run on prod (2026-07-31): terawulf applied alone first as a smoke test (62 → 0, `path_prefixes` untouched at 4, re-run reported 0 changes — the first live exercise of the prefix-collapse branch), then the remainder. **`local_paths` across all projects: 194 → 17.** Full re-run reports `0/35 changes`, confirming idempotency against real data. Snapshots retained on LXC 114 at `/root/local_paths.bak.csv` (34 projects, pre-smoke-test) and `/root/local_paths.preapply.csv` (35 projects incl. `path_prefixes`, immediately pre-apply). Re-verified 2026-08-05 by task audit. The "Task 10 residual risk" section below is historical.
