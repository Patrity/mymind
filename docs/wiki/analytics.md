...
|---|---|---|---|
| `model_prices` | PK `model` | five `numeric` rate columns — `input_cost_per_token`, `output_cost_per_token`, `cache_read_cost_per_token`, `cache_creation_cost_per_token`, `cache_creation_above_1h_cost_per_token` — plus `source`, `fetched_at` | `server/tasks/sync-model-prices.ts`, cron `0 4 * * *`. Mirrors LiteLLM's public `model_prices_and_context_window.json` for every model MyMind has actually seen (`SELECT DISTINCT model FROM messages`) — no reason to store rates for the thousands of models never used. `numeric`, not `real`: rates are ~1e-7 magnitude multiplied by billions of tokens, where float drift would be visible. 8 rows on the local corpus at time of writing — production has an additional priced model (`claude-opus-5`) not present locally, so prod's `model_prices` will have 9 rows; see [Chart colour assignment](#chart-colour-assignment). |
...
**First run after deploy — cold-start window.** Both tables above are populated by cron, not by the deploy itself: `model_prices` first fills at the next `0 4 * * *` run, `litellm_daily` at the next `20 0 * * *` run. Until whichever runs first, `model_prices` is empty, so **every** model falls into the unpriced bucket and the value tile reads **$0.00** — this is expected, not a bug, and can last up to ~24h depending on deploy time relative to the cron schedule. The honest signal during this window is the unpriced-tokens note under the tiles (`UsageTiles.vue`, renders whenever `unpriced.tokens > 0`) — trust that over the $0.00 tile. **There is no manual trigger**: `/_nitro/tasks` (Nitro's task-invocation UI) is dev-only and does not exist in the built production output, and neither task has an admin API endpoint. The only remedies are waiting for the cron to fire or seeding `model_prices` directly (e.g. via `psql`, from the same LiteLLM price map URL — `PRICE_MAP_URL` in `server/lib/analytics/prices.ts`).
...
**Usage tab (cycle 55, shipped):** `server/db/schema/{model-prices,litellm-daily}.ts` · migration `server/db/migrations/0031_zippy_random.sql` · `server/lib/analytics/{cost,prices}.ts` · `server/services/usage.ts` · `server/tasks/{sync-model-prices,rollup-litellm-daily}.ts` · `server/api/analytics/{usage,dispatches}.get.ts` · `shared/types/usage.ts` · `app/components/analytics/{UsageTiles,UsageStackedChart,UsageBreakdownBars}.vue`. Tests: `test/usage-{range,cost,prices}.test.ts` + `test/usage-aggregation.db.test.ts` (DB-tagged, `pnpm test:db` only — not in the CI/deploy gate).
