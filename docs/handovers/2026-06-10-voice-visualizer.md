...
shipped:
  ...
  - "app/pages/voice.vue — passes state/connected/micAnalyser/onVizEvent to Reactor; activeAnalyser helper removed. Acceptance additions: opaque caption over the canvas showing the current turn's message (bg-elevated, line-clamp-3); two sibling UDashboardPanels (documents pattern): canvas panel resizable/sized in % (default 75, min 50, max 90 — the layout's UDashboardGroup now uses the default % unit app-wide; double-click the handle to reset), transcript panel fluid on the right, hidden below lg; both bodies use :ui body !p-0 so the Three.js canvas fills the panel edge-to-edge. NOTE: Nuxt UI's resize handle only supports a sized panel LEFT of the handle (no side prop on UDashboardPanel), which is why the canvas is the sized one."
...
shipped_acceptance_additions:
  ...
  - "Neural lightning (app/lib/viz/lightning.ts): pooled jagged LineSegments arcs with branches firing between points in the core during thinking (firing directive = 1) and simmering during tool (0.35). Knobs in VIZ_TUNING.lightning (rate/brightness/jag)."
  - "STT 415 root-cause fix: crossws@0.3.5's node adapter drops the ws isBinary flag, so JSON control frames arrived as Buffers and were fed to Whisper as audio. server/lib/voice/frames.ts now classifies frames by CONTENT (RIFF→audio, JSON→control; 5 tests). Also: ws.ts catches pipeline failures and emits {type:'error'} to the client (error flash + alert + idle), and the WS upgrade is auth-gated via an upgrade() hook (nitro middleware does NOT run for WS upgrades — the socket was previously unauthenticated)."
  - "Voice settings (app/composables/useVoiceSettings.ts + app/components/voice/SettingsSlideover.vue): cog button opens a USlideover with voice picker, speech-sensitivity slider with LIVE Silero probability meter (useVoice.speechProb via onFrameProcessed — same 0..1 unit as the threshold), barge-in switch, min-speech / end-of-turn sliders, playback speed. Persisted in the voice-settings cookie (useCookie). Threshold/timing changes hot-apply via useVoice.applyVadSettings() (debounced VAD-only restart); barge-in + playback rate read live. Replaces the hardcoded TUNING const and the navbar USelectMenu."
  - "Panel layout: two sibling UDashboardPanels (canvas resizable 75%/50–90% of the content area, transcript fluid right, hidden <lg); UDashboardGroup unit switched from rem to % app-wide. Opaque current-turn caption over the canvas; scroll-wheel camera zoom; VIZ_TUNING knob file (camera/bloom/core/ring/lightning) + PALETTE moved there."
deferred:
  - "Phone (LAN) feel-pass — desktop acceptance is DONE (2026-06-10: Tony tuned bloom/zoom/colors live, exercised voice + typed turns + settings); the phone pass on the LAN is the remaining check (adaptive tier ≈10k particles, touch ergonomics — note: no pinch zoom, wheel only)."
  - "Visualizer THEME settings (palette presets, particle density sliders) — still code-only by design. The settings slideover shipped for capture/playback knobs; visual theming remains in app/lib/viz/tuning.ts."
