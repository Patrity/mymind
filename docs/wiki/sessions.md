...
- **Session columns** (from the `[event]` hook): `machine_id`, `hostname`, `git_branch`, `git_commit`, `git_remote`, `app_version`, plus `ended_at` (set on `SessionEnd`). **`project_id`** (cycle 14) is resolved on ingest via `findOrCreateProject({ gitRemote, cwd })` — canonical match on the normalized git remote (Uncategorized fallback for no-remote); the legacy `project` slug is kept in sync. See [projects.md](projects.md).
