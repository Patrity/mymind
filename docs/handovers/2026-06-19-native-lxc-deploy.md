...
validation:
  ...
  - "**Still pending (Tony, optional):** an **existing uploaded image still loads** + a **new upload persists** (uploads migration); the service **survives an LXC reboot**. App health, DB connectivity, `systemctl active`, and `docker ps` (only db+searxng) already confirmed via SSH."
deferred:
