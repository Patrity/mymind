...
> **Deleted in cycle 60:** `app/components/voice/Reactor.client.vue`, `app/components/agent/HistorySlideover.vue`, `app/lib/viz/ring.ts`, `server/lib/voice/chunker.ts`. `app/components/voice/VoicePicker.vue` was already gone before this cycle (the picker is inline in `SettingsSlideover.vue`); `BAR_COUNT`, `Directives.ringLevels`/`ringColor`, `PALETTE.*.ring` and `VIZ_TUNING.ring` **survive** — the head still samples the mic into `BAR_COUNT` bands and `effects.ts` still uses `VIZ_TUNING.ring.radius` to place the tool-pulse rings.
