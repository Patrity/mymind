...
/**
 * RULING 18 (task-9 fix round 1): a `review` verdict here does NOT file a `review_queue` row
 * for a human — it is a no-op, leaving the memory at its pre-existing `applicability =
 * 'project'` default.
 *
 * Why: over the live store (1,599 memories) the classifier's raw `noul` is compressed in the
 * middle — p25 0.63, median 0.75, p75 0.84 on a 0-1 scale — so only 7.8% of scores fell
 * outside the 0.9/0.1 gate at all. Queueing the other 92.2% put 1,475 low-value
 * `applicability` rows into `review_queue`, burying the surface's other kinds (4
 * memory-contradict, 17 triage, 10 enrichment) under noise none of Tony's review time should
 * go to. `global` promotion is expected to come from the resident-promotion / retrieval-count
 * path instead: a memory that's actually reused across projects is a measured signal, unlike
 * one model's opinion on a distribution too flat to gate confidently.
 */
