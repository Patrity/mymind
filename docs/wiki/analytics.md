...
**status: shipped** (cycle 44 — merged + deployed to prod 2026-07-06; dev live-validated end-to-end against the real homelab). **Usage tab added cycle 55 — ✅ shipped** (merged ff `a24eee4..985b635`, 15 commits; CD run 31861147144 applied migration 0031; prod verified in-container). See the [roadmap](../superpowers/plans/00-roadmap.md) row 55.

A dashboard for the local AI estate, split into two tabs. **Infrastructure** (cycle 44, shipped): per-GPU telemetry on the AI rig, inference-engine activity (vLLM / TEI / llama.cpp), LiteLLM traffic + spend, and a live request log — read-only, everything read server-side from the homelab's existing Prometheus (`192.168.2.90:9090`, Dell LXC 111) and the LiteLLM admin API (`192.168.2.85:4000`), nothing persisted. **Usage** (cycle 55, shipped): historical token consumption and API-equivalent value across Claude Code sessions and LiteLLM spend, sourced from Postgres — see [Usage tab](#usage-tab-cycle-55) below.
...
**✅ shipped 2026-08-14** — merged to master (`a24eee4..985b635`) and deployed by CD run 31861147144, which applied migration `0031`. Verified in prod in-container: both tables present, `drizzle.__drizzle_migrations` 31→32, existing data intact (147,078 messages / 597 sessions), and a live authenticated round-trip on `/api/analytics/usage?range=30d` returning real aggregates (10.9B tokens, 96.76% cache reads, 83 sessions, 1,197 dispatches) with `range=1h` → 400.
...
|---|---|---|---|
| `model_prices` | PK `model` | five `numeric` rate columns — `input_cost_per_token`, `output_cost_per_token`, `cache_read_cost_per_token`, `cache_creation_cost_per_token`, `cache_creation_above_1h_cost_per_token` — plus `source`, `fetched_at` | `server/tasks/sync-model-prices.ts`, cron `*/15 * * * *`. Mirrors LiteLLM's public `model_prices_and_context_window.json` for every model MyMind has actually seen (`SELECT DISTINCT model FROM messages`) — no reason to store rates for the thousands of models never used. **The 15-minute cadence does not mean 15-minute fetches**: the task self-gates via `decideSync` (`server/lib/analytics/prices.ts`) and only hits the network when a model appears that it has never *attempted* to price, or when the last full sync is older than `PRICE_MAX_AGE_MS` (20h). It was `0 4 * * *` until 2026-09-03, when `claude-fable-5-1` sat in the unpriced bucket for ~11h waiting for the next run — the daily cadence was sized for how often *rates* change, but the trigger that actually matters is a **new model appearing in usage**. Trigger state (attempted models + last full sync) lives in `settings` under key `model_price_sync`; see `server/lib/analytics/price-sync-state.ts`. Note the guard keys off **attempted**, not **priced** — `<synthetic>` can never gain a price row, so a priced-based guard would re-fetch every tick forever. `numeric`, not `real`: rates are ~1e-7 magnitude multiplied by billions of tokens, where float drift would be visible. 8 rows on the local corpus at time of writing — production has an additional priced model (`claude-opus-5`) not present locally, so prod's `model_prices` will have 9 rows; see [Chart colour assignment](#chart-colour-assignment). |
...
**First run after deploy — cold-start window.** Both tables above are populated by cron, not by the deploy itself: `model_prices` first fills at the next `*/15 * * * *` run (a null last-sync reads as stale, so the first tick syncs), `litellm_daily` at the next `20 0 * * *` run. Until whichever runs first, `model_prices` is empty, so **every** model falls into the unpriced bucket and the value tile reads **$0.00** — this is expected, not a bug, but since 2026-09-03 it lasts **≤15 minutes**, not the ~24h it could take under the old daily cron. The honest signal during this window is the unpriced-tokens note under the tiles (`UsageTiles.vue`, renders whenever `unpriced.tokens > 0`) — trust that over the $0.00 tile. **There is no manual trigger**: `/_nitro/tasks` (Nitro's task-invocation UI) is dev-only and does not exist in the built production output, and neither task has an admin API endpoint. The only remedies are waiting for the cron to fire or seeding `model_prices` directly (e.g. via `psql`, from the same LiteLLM price map URL — `PRICE_MAP_URL` in `server/lib/analytics/prices.ts`).
...
**The residual term is load-bearing, not padding.** `cache_creation_input_tokens` is the reported total for cache-write tokens; the two ephemeral fields are supposed to split that total by TTL tier, but on the real corpus they don't always sum back to it — 3 of the 81,954 usage-bearing `messages` rows report a non-zero `cache_creation_input_tokens` (3,028 each, 9,084 total) while both ephemeral tiers read 0. Drop the residual term and those tokens silently price at $0. It's priced at the cheaper 5-minute rate (never the 1h rate), so an unknown tier can't inflate the total, and clamped at `≥ 0`, so an over-counted split can't drive the total negative.
...
**Numbers below are the local corpus at time of writing, not production.** Locally, **9** distinct models compete for an **8**-hue palette, so one collision is pigeonhole-forced — no assignment function, deterministic or not, can give 9 items 8 mutually exclusive colours; here that's `claude-opus-4-6` and `claude-sonnet-4-6`, both landing on slot 2. Production has a **10th** model the local corpus has never seen (`claude-opus-5` — 11,991 rows, 4.35B tokens — already present in the price map), so on prod the palette is short by two against 10 models, forcing **two** collisions rather than one. This is expected and disclosed, not a bug to chase; it recurs any time the model count exceeds 8, and which specific pair(s) collide depends on the full model set's hash distribution.
...
**Usage tab (cycle 55, shipped):** `server/db/schema/{model-prices,litellm-daily}.ts` · migration `server/db/migrations/0031_zippy_random.sql` · `server/lib/analytics/{cost,prices,price-sync-state}.ts` · `server/services/usage.ts` · `server/tasks/{sync-model-prices,rollup-litellm-daily}.ts` · `server/api/analytics/{usage,dispatches}.get.ts` · `shared/types/usage.ts` · `app/components/analytics/{UsageTiles,UsageStackedChart,UsageBreakdownBars}.vue`. Tests: `test/usage-{range,cost,prices}.test.ts` + `test/model-price-sync-trigger.test.ts` + `test/{usage-aggregation,price-sync-state}.db.test.ts` (DB-tagged, `pnpm test:db` only — not in the CI/deploy gate).
