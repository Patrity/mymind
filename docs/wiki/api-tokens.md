...
- **Hooks (Windows)**: `Invoke-WebRequest "$env:MYMIND_URL/api/setup/cc-hook.ps1" -OutFile "$HOME\.mymind\cc-hook.ps1"`, then a `%USERPROFILE%\.claude\settings.json` block wiring `powershell -NoProfile -ExecutionPolicy Bypass -File "C:\Users\YOU\.mymind\cc-hook.ps1" <Event>`. The path **must be absolute**: Claude Code does not expand `%USERPROFILE%` (nor `~`) inside a hook command on Windows — the literal string reaches `powershell.exe` and the hook dies with `The argument '%USERPROFILE%\.mymind\cc-hook.ps1' to the -File parameter does not exist`. Step 3.2 on the Connect tab prints the whole block with `$HOME` already resolved. (Running CC under WSL → use the macOS/Linux tab.)
