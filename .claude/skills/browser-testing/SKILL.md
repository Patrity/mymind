...
`UTabs`, `USelectMenu`, `USwitch`, segmented controls (reka-ui) **must** be driven with `playwright-cli click <e-ref>` (a real click after `snapshot`). A programmatic `el.click()` inside `eval` does NOT fire reka's handler — the tab/toggle won't change. (Memory: `playwright-cli-reka-tabs`.) Note: `UTabs` renders all panel *wrappers* and marks inactive ones (`[hidden]`/`data-state`), so query the **active** panel (`[role=tabpanel][data-state=active]`), not the first one — but the wrapper is all that survives. **Inactive tab CONTENT is unmounted by default** (`unmountOnHide: true` → a `v-if` on reka's `TabsContent` slot), so an inactive panel has `childCount: 0` and **no component state inside a tab survives a tab switch** — a running request is orphaned, refs reset, `onBeforeUnmount` cleanup fires. Check `childCount` before concluding a control is "missing". The opt-out is `:unmount-on-hide="false"` (bound, not static — the prop is typed Boolean, so `unmount-on-hide="false"` passes the truthy string); cycle 67 needed it after a refactor moved live state into the slots.
