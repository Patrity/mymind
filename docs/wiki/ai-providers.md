...
| `POST /api/settings/import-env` | One-time onboarding seed from leftover `AI_*` env (see Migration). |
...
`POST /api/settings/import-env` (`import-env.post.ts`) reads `process.env` directly, maps the old roles → usages (`AI_REASONING`→reasoning, `AI_BULK`→bulk, `AI_EMBEDDINGS`→embeddings (dim 2560), `AI_VISION`→vision, `AI_STT`→stt, `AI_TTS_KOKORO` + `AI_TTS_CHATTERBOX`→tts, `AI_RERANK`→rerank), dedupes providers by baseURL, encrypts keys, and saves. **Known consideration:** it builds from `emptyDoc()` and **overwrites the whole `ai_config` row** — it is a seed for an empty config, surfaced only via the onboarding Import button, not a merge. After onboarding, the `AI_*` vars do nothing at runtime; all model config lives in the DB and is edited in `/settings`.
