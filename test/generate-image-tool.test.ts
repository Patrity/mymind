...
describe('generate_image tool', () => {
  ...
  it('generates, persists, publishes, returns image_id (no url to model) + display images, with undo', async () => {
  ...
  it('generates n images sequentially', async () => {
    ...
    const exec2 = exec as { display?: { images: unknown[] } }
    expect(exec2.display!.images.length).toBe(2)
  ...
  it('partial success: keeps the first image when a later generation fails (with undo)', async () => {
    ...
    const exec = await tool.handler({ prompt: 'x', n: 2 }, ctx)
    const exec2 = exec as { display?: { images: { id: string }[] } }
    expect(exec2.display!.images.length).toBe(1)
    expect(exec2.display!.images[0].id).toBe('a')
  ...
  it('aborts before generating when the signal is already aborted', async () => {
    ...
    const exec2 = exec as { display?: { images: unknown[] } }
    expect(exec2.display!.images.length).toBe(0)
  })
